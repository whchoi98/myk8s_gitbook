...
Cloud9 이 올바른 IAM 역할을 사용하고 있는지 확인합니다. \(IAM Role 이름을 "eksworkshop-admin"으로 선언하지 않은 경우에는 다른 이름으로 변경합니다.\)
...
생성된 SSH key를  Key 페어로 전송합니다. 앞서 eksworkshop.pub 로 public key가 생성되었습니다. \(AWS CLI Version 2.0\)
...
OpenSSH public key format 에러가 발생할 경우 아래와 같은 명령으로 Key 전송합니다. \(AWS CLI version 1.x\)
